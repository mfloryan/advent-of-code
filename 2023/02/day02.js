    ...
    let sets = g[1]
        ...
        .map(g => 
            g.split(", ")
             .map(_ => {
                box = _.trim().split(' ');
                return [parseInt(box[0]), box[1]] })
             .reduce((p, c) => { p[c[1]] = c[0]; return p }, {}))
    return {
        id,
        sets
    }
...
let x = games.map(minimumBag).map(_ => Object.values(_).reduce((p, c) => p * c, 1))
console.log(x.reduce((p, c) => p + c, 0))
